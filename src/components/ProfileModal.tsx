...
import { X, Heart, MessageSquare, Sparkles, Flag, Ban, MapPin, ShieldCheck, Lock, Unlock, Zap, Flame, Hand, Check } from 'lucide-react';
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-3 gap-2">
              {publicPhotos.slice(1).map(p => (
                <div key={p.id} className="relative aspect-square rounded-2xl overflow-hidden border border-white/10 bg-black shadow-md">
                  <img src={p.url} alt="Gallery photo" referrerPolicy="no-referrer" className="w-full h-full object-cover" />
                </div>
              ))}
            </div>
          </div>
        ...
        {privatePhotos.length > 0 && (
          <div className="rounded-[24px] border border-amber-500/30 bg-gradient-to-br from-amber-950/30 via-black to-purple-950/20 p-4 space-y-3 shadow-xl">
            ...
            {hasLockedVault ? (
              ...
              <div className="grid grid-cols-3 gap-2">
                {privatePhotos.map(p => (
                  <div key={p.id} className="relative aspect-square rounded-2xl overflow-hidden border border-amber-500/40 bg-black shadow-md">
                    <img src={p.url} alt="Vault photo" referrerPolicy="no-referrer" className="w-full h-full object-cover" />
                  </div>
                ))}
              </div>
            )}
